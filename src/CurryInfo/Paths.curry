...
getDirPath :: QueryObject -> IO String
getDirPath (PackageObject pkg) = do
  path <- packagesPath
  return (path </> pkg)
getDirPath (VersionObject pkg vsn) = do
  path <- getDirPath (PackageObject pkg)
  return (path </> "versions" </> vsn)
getDirPath (ModuleObject pkg vsn m) = do
  path <- getDirPath (VersionObject pkg vsn)
  return (path </> "modules" </> m)
getDirPath (TypeObject pkg vsn m _) = do
  path <- getDirPath (ModuleObject pkg vsn m)
  return (path </> "types")
getDirPath (TypeClassObject pkg vsn m _) = do
  path <- getDirPath (ModuleObject pkg vsn m)
  return (path </> "typeclasses")
getDirPath (OperationObject pkg vsn m _) = do
  path <- getDirPath (ModuleObject pkg vsn m)
  return (path </> "operations")
